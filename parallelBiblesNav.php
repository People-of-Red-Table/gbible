		...
		<!-- Bible 1 -->
			...
			<form method="post" name="countrySelectionForm1">
				...
				<div class="form-group">
					...
					<select class="bible-nav-select form-control" name="country1" onchange="document.countrySelectionForm1.submit();" id="countryOfCountrySelectionForm1">
							<?php
									...
									else
									{
										...
										$result_country = $statement_country -> execute(array('country' => $userBibleA -> country));
				...
				<form method="post" name="languageSelectionForm1">
					<input type="hidden" name="menu" value="parallelBibles" />
					<input type="hidden" name="country1" value="<?=$country1;?>" id="countryOfLanguageSelectionForm1" />
				<div class="form-group">
						<label for="languageOfLanguageSelectionForm1"><?=$text['text_language'];?></label>
						<select id="languageOfLanguageSelectionForm1" class="form-control" name="language1" onchange="document.languageSelectionForm1.country1.value = document.countrySelectionForm1.country1.value; document.languageSelectionForm1.submit();">
							<?php
									...
									else
									{
										$statement_language = $links['sofia']['pdo'] -> prepare('
											...
											');
										$result = $statement_language -> execute(array('language' => $userBibleA -> language, 'country' => $country_name));
										if (!$result)
										{
				...
				<div class="form-group">
						...
						<input type="hidden" name="language1" value="<?=$language1;?>" />
						<select id="bible-selection-1" class="form-control" name="b_code1" onchange="document.bibleSelectionForm1.country1.value = document.countrySelectionForm1.country1.value; bibleSelectionForm1.language1.value = document.languageSelectionForm1.language1.value; document.bibleSelectionForm1.submit();">
							<?php
									...
									else
									{
										...
										{
											$selected = '';
											if (isset($b_code1) and (strcasecmp($b_code1, $row['b_code']) ===0))
											{
												$selected = ' selected="selected"'; 
											}
	...
	<div class="col-md-6 hidden-print">
		...
		<form method="post" name="countrySelectionForm2">
				...
				<div class="form-group">
					...
					<select class="bible-nav-select form-control" name="country2" onchange="document.countrySelectionForm2.submit();" id="countryOfCountrySelectionForm2">
							<?php
									$query = 'select distinct country `country_name`, 
												case when cnt.native is null then t.country else cnt.native end `country`
										from
									...
									{
										$statement_country = $links['sofia']['pdo'] -> prepare('select country_name `country` from iso_ms_languages where country_code = :country or country_name = :country');
										$result_country = $statement_country -> execute(array('country' => $userBibleB -> country));
										//print_r($statement_country);
										if (!$result_country)
										{	
											log_msg(__FILE__ . ':' . __LINE__ . ' Countries PDO query exception. Info = {' . json_encode($statement_country -> errorInfo()) . '}, $_REQUEST = {' . json_encode($_REQUEST) . '}');
				...
				<form method="post" name="languageSelectionForm2">
					<input type="hidden" name="menu" value="parallelBibles" />
					<input type="hidden" name="country2" value="<?=$country2;?>" id="countryOfLanguageSelectionForm2" />
				<div class="form-group">
						<label for="languageOfLanguageSelectionForm2"><?=$text['text_language'];?></label>
						<select id="languageOfLanguageSelectionForm2" class="form-control" name="language2" onchange="document.languageSelectionForm2.country2.value = document.countrySelectionForm2.country2.value; document.languageSelectionForm2.submit();">
							<?php
									$statement_languages = $links['sofia']['pdo'] -> prepare(
											'
												select distinct t.language_name, case when iso_ms.native_language_name is null then t.language_name else iso_ms.native_language_name end `native_language_name` from
									...
									else
									{
										$statement_language = $links['sofia']['pdo'] -> prepare('
											...
											select language `language_name` from b_shelf where country = :country
											');
										$result = $statement_language -> execute(array('language' => $userBibleB -> language, 'country' => $country_name));
										if (!$result)
										{
											log_msg(__FILE__ . ':' . __LINE__ . ' Languages PDO query exception. Info = {' . json_encode($statement_language -> errorInfo()) . '}, $_REQUEST = {' . json_encode($_REQUEST) . '}');
											//echo "Whoops. We've got issue with PDO connection... Sorry. Please contact support.";
										...
										foreach ($language_rows as $item) 
										{
											if (strcasecmp($item['language_name'], $userBibleB -> language) === 0)
				...
				<div class="form-group">
						...
						<input type="hidden" name="language2" value="<?=$language2;?>" />
						<select id="bible-selection-2" class="form-control" name="b_code2" onchange="document.bibleSelectionForm2.country2.value = document.countrySelectionForm2.country2.value; bibleSelectionForm2.language2.value = document.languageSelectionForm2.language2.value; document.bibleSelectionForm2.submit();">
							<?php
									$statement_bibles = $links['sofia']['pdo'] -> prepare(
											'select b_code, title from b_shelf where language = :language_name order by title');
										$result_bibles = $statement_bibles -> execute(array('language_name' => $language_name));
									...
									else
									{
										...
										{
											$selected = '';
											if (isset($b_code2) and (strcasecmp($b_code2, $row['b_code']) === 0) )
											{
												$selected = ' selected="selected"'; 
											}
											echo '<option value="' . $row['b_code'] . '"' . $selected . '>' . $row['title'] . '</option>';
