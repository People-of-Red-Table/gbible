<?php
	...
	$statement_chapters = $pdo -> prepare('select distinct book, chapter from ' .  $tt_userBibleA -> table_name . '
											where book in (select distinct book from ' . $tt_userBibleB -> table_name . ' )');
	...
	$insert_timetable_query = 'insert into timetables (user_id, title, b_code, b_code2, scheduled) values (:user_id, :title, :b_code, :b_code2, now());';
	...
	if ($result)
	{
		...
		if ($result)
		{
			...
			if (!$insert_result)
			{
				$message = ['type' => 'danger', 'message' => $text['timetable_create_def_tt_exception']];
